...
export function RippleFlagCanvas({ onClose, isWindow = false }: RippleFlagCanvasProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#09090D] text-white rounded-lg overflow-hidden select-none font-sans shadow-2xl border border-white/10">
      {/* Title Bar */}
      {isWindow && (
        <div className="h-10 bg-[#141419] border-b border-white/10 flex items-center justify-between px-3.5 select-none flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-white tracking-tight ml-2 flex items-center gap-1.5">
              ...
              <span>Fluid Ripple Flag Shader — Pensatori Irrazionali Engine</span>
            </span>
          </div>
          <span className="text-[11px] font-mono text-zinc-400">Move mouse to ripple silk</span>
        </div>
      )}
